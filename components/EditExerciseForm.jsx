...
export default function EditExerciseForm({ exercise, onSave, onCancel }) {
  ...
  const existingType = exercise?.type || 'compound';

  function handleSubmit(e) {
    e.preventDefault();
    if (!name.trim()) return;
    onSave({ name: name.trim(), sets: Number(sets), reps, type: existingType });
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="py-4 border-b border-[#2A2A2A]">
      ...
      <div className="flex gap-3">
        <button
          type="submit"
          className="cursor-pointer font-[family-name:var(--font-body)] uppercase tracking-widest px-4 py-2 bg-[#D1E231] text-[#0F0F0F] font-bold transition-opacity hover:opacity-80"
        ...
        <button
          type="button"
          onClick={onCancel}
          className="cursor-pointer font-[family-name:var(--font-body)] uppercase tracking-widest px-4 py-2 border border-[#2A2A2A] text-[#888780] hover:border-[#D1E231] hover:text-[#D1E231] transition-colors"
          style={{ fontSize: 'clamp(0.6rem, 2.5vw, 0.7rem)' }}
        >
          Cancel
        </button>
